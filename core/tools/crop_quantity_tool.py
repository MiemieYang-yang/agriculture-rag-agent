"""
虚谷数据库查询工具
查询作物品质数据表 AGME_SUB_AGI_QUALITY_TYPE_TAB
"""
...
class crop_quantity_tool(BaseTool):
    """虚谷数据库查询工具 - 查询作物品质数据"""
    ...
    def description(self) -> str:
        return (
            "使用模糊查询规范的作物品质数据，如提问品质，则优先使用该工具。"
            "可按作物名称等条件筛选，返回启用的品质表格数据。"
    ...
    def _build_query(self, crop_name: Optional[str], limit: int) -> str:
        ...
        base_query = """
            ...
            V_QUA_NAME,
            V_QUA_UNIT
            FROM AGME_SUB_AGI_QUALITY_TYPE_TAB
            WHERE IS_USED = 1
        """
    ...
    def _generate_summary(self, results: List[Dict], crop_name: Optional[str]) -> str:
        ...
        if not results:
            if crop_name:
                return f"未找到作物「{crop_name}」的品质数据。"
            return "未找到任何品质数据。"

        crop_filter = f"作物「{crop_name}」的" if crop_name else ""
        summary_parts = [f"查询到 {len(results)} 条{crop_filter}品质数据："]
