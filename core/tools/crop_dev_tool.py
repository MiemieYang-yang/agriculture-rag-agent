...
class crop_dev_tool(BaseTool):
    ...
    @property
    def name(self) -> str:
        return "query_crop_development"

    @property
    def description(self) -> str:
        return (
            "使用模糊查询规范的作物发育期数据，如提问发育期，则优先使用该工具。"
            "可按作物名称等条件筛选，返回启用的发育期表格数据。"
    ...
    def _generate_summary(self, results: List[Dict], crop_name: Optional[str]) -> str:
        ...
        if not results:
            if crop_name:
                return f"未找到作物「{crop_name}」的发育期数据。"
            return "未找到任何发育期数据。"

        crop_filter = f"作物「{crop_name}」的" if crop_name else ""
        summary_parts = [f"查询到 {len(results)} 条{crop_filter}发育期数据："]
